// Background service worker — 10-tab worker pool for Zilla Parishad
...
async function getSeriesStructure() {
    const proj = JSON.stringify({ details: { id: 1, name: 1, sections: { id: 1, name: 1, subsections: { id: 1, name: 1 }, paidTestCount: 1, freeTestCount: 1 } } });
    ...
    return new Promise((resolve) => {
        ...
        chrome.tabs.create({ url, active: false }, (tab) => {
            ...
            function handler(msg, sender) {
                ...
                if (msg.type === "submitResult" && !settled) { settled = true; cleanup(); chrome.tabs.remove(tabId).catch(() => { }); resolve(msg.result); }
                if (msg.type === "pageReady") { chrome.tabs.sendMessage(tabId, { action: "autoSubmit", testId: test.id, title: test.title }).catch(() => { }); }
            }
...
async function worker() {
    while (testQueue.length > 0 && running) {
        ...
        if (done) { stats.skipped++; activeWorkers--; updateProgress(); continue; }
        const result = await processOneTest(test);
        if (result === "ok") stats.submitted++; else stats.failed++;
        activeWorkers--;
        updateProgress();
    }
}
